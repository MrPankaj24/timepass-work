...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
/* A copy of alloc_vfsmnt() but allocates the fake mnt_id for mounts that are unshared by ksu process */
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
/* A copy of alloc_vfsmnt() but allocates the fake mnt_id for mount that is mounted or single cloned by ksu process */
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	// Skip ksu domain checks after boot-completed to reduce overhead
	if (susfs_is_sdcard_android_data_decrypted) {
		goto skip_checking_for_ksu_proc;
	}

	// Check for ksu process first because of magic mount
	if (susfs_is_current_ksu_domain()) {
		// if it is unsharing, allocate from DEFAULT_UNSHARE_KSU_MNT_ID
		if (flag & CL_COPY_MNT_NS) {
			mnt = susfs_alloc_unshare_ksu_vfsmnt(old->mnt_devname);
			goto bypass_orig_flow;
		}
		// else allocate from DEFAULT_KSU_MNT_ID
		mnt = susfs_alloc_non_unshare_ksu_vfsmnt(old->mnt_devname);
		goto bypass_orig_flow;
	}

skip_checking_for_ksu_proc:
	// For other processes cloning a ksu mount, allocate from DEFAULT_KSU_MNT_ID
